package ru.gpb.grules;

import ru.gpb.grules.businessModel.*;
...
import ru.gpb.grules.ruleEngine.Performable;
...
public class Console {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, NoSuchFieldException, ParseException, InvocationTargetException, IllegalAccessException {
        ...
        console.readDir();
        CreditRequest creditRequest = console.initRequest();
        while (!creditRequest.isComplete()) {
            console.complete(creditRequest);
            System.out.println(creditRequest);
            if (creditRequest.isComplete()) {
                console.perform(creditRequest);
                console.clean(creditRequest);
            }
        }
    }
    ...
    void readDir() throws IOException, NoSuchFieldException, ClassNotFoundException, ParseException {
        String basePath = Console.class.getClassLoader().getResource("Rules").getPath();
